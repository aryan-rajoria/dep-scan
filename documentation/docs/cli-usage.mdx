...
options:
  ...
  --severity {low,medium,high,critical}
                        Minimum severity threshold to report. Only advisories at or above this severity are returned. On the default (non-extended) vdb this is a safe no-op: depscan keeps its own severity gating authoritative because severity metadata may be absent. Full severity filtering requires the extended vdb, enabled with VDB_INCLUDE_METADATA=true.
