...
        A["Get Args"] --> B{"Debug Mode?"};
        B -- Yes --> C["Enable Debug"];
        B -- No --> D{"Server Mode?"};
        D -- Yes --> E["Run Server"];
        D -- No --> F["Print Logo"];
        A --> G["Determine src_dir"];
        A --> H{"CSAF Enabled?"};
        H -- Yes --> I["Check/Create csaf.toml"];
    end
    
    subgraph Project Analysis
        G --> J["Detect Project Types"];
        J --> K{"Single PURL?"};
        K -- Yes --> L["Enable Risk Audit"];
        K -- No --> M["Create BOM"];
        M --> N{"BOM Created?"};
        N -- Yes --> O["Get Package List"];
        N -- No --> P["Continue to Next Type"];
        O --> Q["License Analysis"];
        O --> R{"Risk Audit Enabled?"};
        R -- Yes --> S["Perform Risk Audit"];
        R -- No --> T["Report Risk Audit Capability"];
        O --> U["Remote Audit"];
        O --> V["Vulnerability Search"];
    end

    subgraph Reporting & Submission
        V --> W["Summarize Results"];
        W --> X["Generate Reports (HTML, PDF)"];
        W --> Y{"Custom Report Template?"};
        Y -- Yes --> Z["Render Custom Report"];
        W --> AA["Explain Results"];
        W --> BB{"CSAF Enabled?"};
        BB -- Yes --> CC["Export CSAF"];
        X --> DD["Submit to ThreatDB"];
    end
...
    style ZZZ fill:#ccf,stroke:#333,stroke-width:2px
