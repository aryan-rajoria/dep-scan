...
- VDB_INCLUDE_METADATA - Set to true to download and use the extended vulnerability metadata. Required for full `--severity` filtering inside the database, because severity, package scope, date, and source fields are only reliably populated with the extended tables. When this is unset, `--severity` still filters the final report: depscan applies the threshold to the assembled VDR itself, so the floor is honored on any database and is never silently ignored.
...
- DEPSCAN_SEVERITY - Alternative to the `--severity` argument. Set to `low`, `medium`, `high`, or `critical` to apply a minimum severity floor to the report. The floor is enforced on any database; `VDB_INCLUDE_METADATA=true` additionally pushes it into the search for speed.
