...
- DEPSCAN_SEVERITY - Alternative to the `--severity` argument. Set to `low`, `medium`, `high`, or `critical` to push a minimum severity threshold down to the vulnerability database. Requires `VDB_INCLUDE_METADATA=true` for full filtering.
